...
func ObjectRetentionPut(c *cli.Context) (err error) {
	...
	input := new(s3.PutObjectRetentionInput)

	// Required parameters
	mandatory := map[string]string{
		fields.Bucket:     flags.Bucket,
		fields.Key:        flags.Key,
		fields.Retention:  flags.Retention,
	}
	...
	// PutObjectRetention Op
	var output *s3.PutObjectRetentionOutput
	if output, err = client.PutObjectRetention(input); err != nil {
		return
	}
