...
func ObjectLegalHoldPut(c *cli.Context) (err error) {
	...
	input := new(s3.PutObjectLegalHoldInput)

	// Required parameters
	mandatory := map[string]string{
		fields.Bucket:                   flags.Bucket,
		fields.Key:                      flags.Key,
		fields.LegalHold:                flags.LegalHold,
	}
	...
	// PutObjectLegalHold Op
	var output *s3.PutObjectLegalHoldOutput
	if output, err = client.PutObjectLegalHold(input); err != nil {
		return
	}
